...
def print_stats(all_stats, sel_stats, col_stats, out=sys.stdout):
    ...
    if len(all_stats) > 1:
        ...
        for col_stat in col_stats:
            ...
            print(col_stat, "[mean= ", median(collect),
                  ",stdev=", stdev(collect), "]", file=out)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Utility for executing multiple AFL instances and/or collecting results')
    parser.add_argument('-m', "--mode", type=str, default="ec",
                        choices=["ec", "c", "e"], help="mode of operation")
    parser.add_argument('-c', "--command", type=str,
                        help="command to launch AFL on the binary minus the output directory")
    parser.add_argument('-o', "--output", type=str,
                        help="experiment output dir", required=True)
    parser.add_argument('-t', "--timeout", type=int,
                        default=10, help="timeout for each experiment")
    parser.add_argument('-n', "--numexp", type=int,
                        default=2, help="number of experiments")
    ...
    out_dirs = [os.path.join(args.output, "output_"+str(i))
                for i in range(1, args.numexp+1)]
    ...
    if do_collect:
        ...
        all_stats = [parse_stats(out_dir)
                     for out_dir in out_dirs if os.path.exists(out_dir)]
        with open(os.path.join(args.output, "all_stats"), "w") as sfile:
            print_stats(all_stats, ["paths_favored", "paths_total", "unique_crashes", "unique_hangs", "bitmap_cvg", "execs_per_sec"], [
                        "paths_favored", "paths_total", "unique_crashes", "unique_hangs", "bitmap_cvg", "execs_per_sec"], out=sfile)
